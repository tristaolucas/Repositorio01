...
def main():
    ...
    # Load config
    config = {}
    # Esta instalacao do BMAD nao gera _bmad/config.yaml na raiz: os valores
    # ficam em _bmad/<modulo>/config.yaml. Lemos a raiz (se existir) e depois
    # o core, para nao cair no default "friend"/"English".
    for config_file in ["config.yaml", "config.user.yaml",
                        "core/config.yaml", "core/config.user.yaml"]:
